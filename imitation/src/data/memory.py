...
class ExperienceReplay:
    ...
    def _retrieve_batch(self, idxs: np.ndarray, batch_size=cfg.batch_size):
        vec_idxs = idxs.reshape(-1)
        o_batch = torch.tensor(self.O[vec_idxs]).view(batch_size, cfg.chunk_size, *self.O.shape[1:])
        a_batch = torch.tensor(self.A[vec_idxs]).view(batch_size, cfg.chunk_size, -1)
        r_batch = torch.tensor(self.R[vec_idxs]).view(batch_size, cfg.chunk_size)
        m_batch = torch.tensor(
            [np.all(self.M[vec_idxs].reshape(batch_size, cfg.chunk_size, 1)[:, :ii + 1], axis=1)
