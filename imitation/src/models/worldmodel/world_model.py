...
from src.utils.tools import pyout, init_weights
...
class TransitionModel(nn.Module):
    ...
    def forward(self, s_0: Tensor, A: Tensor, b_0: Tensor, O: Optional[Tensor] = None, M: Optional[Tensor] = None):
        ...
        T = A.size(1) + 1  # minibatchsize and number timesteps
        B = [torch.empty(0)] * T
        ...
        for t in range(T - 1):
            ...
            if O is not None:
                ...
                MU_pos[t + 1], h = torch.chunk(
                    self.line_Spos(self.func(self.line_Bpos(torch.cat([B[t + 1], O[:, t_ + 1]], dim=1)))), 2,
                    dim=1)
